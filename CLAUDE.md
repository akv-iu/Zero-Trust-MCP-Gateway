...
- **Build order** is `PLAN.md` §4.2. Built so far: foundation, 10 (fixture), 11 (harness skeleton), 09 (audit), 01 (edge + bridge), 02 (protocol guard), 03 (identity), 04 (registry), 05 (canonicalizer), 06 (policy broker + Rego), 07 (router — **built without tests, which are owed**). Next is 08. Remaining stubs raise `NotImplementedError` with their owning unit named — **implement the body, keep the signature**.
- **Stage 07 is the only code that acts, and unit 08 is a stub, so the end-to-end path is currently broken in a specific way**: an allowed call reaches the child, causes its side effect, and then dies on `response.validate`. Do not treat that as a router bug; it closes when 08 lands.
