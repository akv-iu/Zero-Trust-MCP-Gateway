...
- **Build order** is `PLAN.md` §4.2. Built so far: foundation, 10 (fixture), 11 (harness skeleton), 09 (audit), 01 (edge + bridge), 02 (protocol guard), 03 (identity), 04 (registry), 05 (canonicalizer), 06 (policy broker + Rego). Next is 07. Remaining stubs raise `NotImplementedError` with their owning unit named — **implement the body, keep the signature**.
