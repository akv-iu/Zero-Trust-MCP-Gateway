...
python -m scripts.run_corpus --profile full      # all 118 rows; required for any published number
...
- **A test that skips on Windows is unverified code, not passing code.** The first CI run on Linux failed three of them, and all three were wrong in the *test*: `CANON_SYMLINK_ESCAPE` expected for a symlink that leaves the tree and comes back **in** (CANON-007 checks containment against the final resolved path, so nothing escaped); `CANON_RESOLUTION_FAILED` expected where containment-before-existence gives `CANON_OUTSIDE_ROOT`; and a symlink path built one directory too deep, which had never raised because it had never run. Before trusting anything guarded by `links_available()` or `_folds_case()`, run it on Linux — `wsl.exe -e bash -lc "cd ~/ztmg && ..."` against a **native ext4 copy**, because `/mnt/*` is DrvFs and silently reports symlinks unavailable, reproducing the Windows skip instead of the Linux run. Fetch OPA there too, or 44 more tests skip and you have measured nothing about them either.
- `harness/scenarios/*.toml` are **data, not test code** — the corpus is a publishable deliverable. TOML cannot carry control characters, so use the `{NUL}` / `{CR}` / `{LF}` / `{TAB}` / `{DEL}` placeholders, expanded at load. `expected_reason` is validated against `ReasonCode` **at load**, because the alternative is finding a typo minutes into a scored run, in whichever of the three modes happens to reach that row.
- **Both slow runners default to a subset, and the subset can never become evidence.** `run_corpus --profile smoke` scores a deterministic 50 rows (~45 s) against `full`'s 118 (~51 s); `run_generated --profile dev` runs 250 cases (~26 s) against `release`'s 25,000 (hours — the two-hour local timeout that is *not* claimed in the README). The profile is written into the artifact and `harness.report` refuses anything but `full`, including a *missing* profile — defaulting the unknown to the permissive answer is how a subset score gets published. Never quote a smoke number anywhere but a terminal.
- **Boot time and scoring time are reported separately** by `run_corpus`, because their fixes differ: boot comes down by needing fewer deployment variants (14 today, 11 serving one row each), scoring by needing fewer rows. A single wall-clock number sends you to optimise whichever half you guessed.
...
- **Build order** is `PLAN.md` §4.2. Built so far: foundation, 10 (fixture), 09 (audit), 01 (edge + bridge), 02 (protocol guard), 03 (identity), 04 (registry), 05 (canonicalizer), 06 (policy broker + Rego), 07 (router), 08 (response guard), and 11 (evaluation harness). **All units 01–11 are done**, each through the `/unit-review` gate. Unit 11's current evidence is 115 PASS / 3 declared skips over 118 protected scenarios, 2,500/2,500 CI-profile generated cases from seed 11011, 113/113 audit joins, and the paired benchmark/report. The 25,000-case release profile hit a two-hour local timeout and is not claimed. There are **no `NotImplementedError` stubs left**.
...
- **The response size ceiling lives in unit 08 alone, and unit 08 reads it off `RawResult.obligations`.** Unit 07 measures and clamps; it does not enforce — it once compared the same number to the same limit one stage earlier, which made `RESP_TOO_LARGE` unreachable. The clamped value then has to *travel*: `pipeline.handle` used to pass `dec.obligations`, so unit 08 enforced what policy asked for while the audit recorded what unit 07 applied. `response.validate` takes no obligations parameter, which is what makes passing the wrong one unrepresentable.
- **A stage's own test scores that stage's decisions only.** `test_protocol.py` and `test_protocol_over_http.py` drive unit 02 alone, so a `layer = "protocol"` corpus row expecting a later stage's code (`REG_ARGS_UNKNOWN_FIELD` on the structural-boundary rows) must be asserted to SURVIVE the guard, never to be denied by it. Asserting a denial there fails correct rows and would pass the day unit 02 started over-rejecting.
