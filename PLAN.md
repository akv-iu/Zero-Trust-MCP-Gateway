...
| 9 | `06-svc-policy-broker` | 03, 04, 05, 09 | OPA allow/deny with reason code; OPA killed → all protected calls denied; **carries spec-03 test 6** (two principals → different decisions, same request) and publishes `IdentityConfig.role_vocabulary` to OPA as data rather than duplicating it. Also owes stage 06's half of the tool-less R0 target, and consumes the per-root operation flags in `[[canonicalize.roots]]` — unit 05 reports them, it does not enforce them | next |
