...
| 10 | `07-svc-upstream-router` | 06, 10 | Only allowed calls reach the fixture; obligations enforced. Also owes REG-010's other half: nothing yet FILTERS the `tools/list` response, so `data.gateway.discoverable` has Rego tests and no Python caller | next |
