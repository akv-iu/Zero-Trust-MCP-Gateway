...
| 11 | `08-svc-response-guard` | 07 | Oversized/mismatched upstream responses become controlled errors. **Blocks the end-to-end path**: stage 07 now forwards, so an allowed call causes its side effect and then dies on `response.validate`'s `NotImplementedError`. The `upstream_attempt` record is what keeps that honest until this lands | next |
