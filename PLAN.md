...
**What each review round found is in [docs/review-log.md](docs/review-log.md)**, not here. It used to live in this section and grew to three times the length of the table it was annotating, which is how row 12 came to carry numbers three revisions stale while the README carried the current ones. The table is the status; the log is the history; measured numbers come from the artifacts.

### Run profiles

The corpus and the generator each have a fast lane, because the full versions are the two slowest things in the project and most of a development session re-proves what did not change.

| Command | Fast lane (default) | Full |
|---|---|---|
| `scripts.run_corpus` | `--profile smoke` — 50 rows, **~45 s** protected | `--profile full` — 118 rows, **~51 s** protected |
| `scripts.run_generated` | `--profile dev` — 250 cases, **~26 s** | `--profile release` — 25,000 cases, **hours** |

The smoke 50 are chosen coverage-greedily over `(layer, expected_reason, fixture_mode, gateway_fault)` in id order: deterministic, and covering all 35 reason codes, all 11 fixture modes and all 3 principals. `harness.report` refuses any artifact whose `profile` is not `full`, and refuses a *missing* profile rather than assuming it — so a subset score cannot reach a published table by accident.

Boot and scoring are timed separately in the output. They have different fixes: boot time comes down by needing fewer deployment variants (14 today, 11 of them serving a single row each), scoring time by needing fewer rows.
