...
| 12 | `11-svc-eval-harness` (full) | all | 100+ scenarios, `direct` vs `protected`, overhead numbers, report. Now unblocked in both directions: the pipeline completes, so `protected` mode can finally be scored against the same corpus `direct` is | next |
