...
| **07** | Upstream cancellation is transport-asymmetric: the client cancels by closing the SSE stream, and `notifications/cancelled` is mandatory on the stdio leg. Resolves **S-5**. *Applied, with a correction: the gateway does not perform the translation — the SDK already does, on a shielded write, with the id it put on the wire. The gateway holds only the CLIENT's id, so translating it here would name an id the child never saw. `bridge.UpstreamHandle.cancel` was deleted rather than wired; see `_specs/90` §10h.* |
