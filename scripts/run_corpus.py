...
`--protected` arrives with unit 11's ProtectedClient. Until then `direct` is the
point: it establishes the "before" picture the gateway must later reduce to zero.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.break_enforcer:
        ...
        client, label = StubEnforcer(containment_enabled=False), "BROKEN ENFORCER"
    else:
